...
import architecture.resonator_combat_framework.init.RcfEntityStates
...
class EntityStateHolder<T : LivingEntity>(
	...
	companion object {
		@JvmStatic
		fun <T : LivingEntity> of(entity: T): EntityStateHolder<T> {
			return entity.getData(RcfAttachmentTypes.ENTITY_STATE_HOLDER) as EntityStateHolder<T>
		}

		@JvmStatic
		fun has(entity: LivingEntity): Boolean {
			return entity.hasData(RcfAttachmentTypes.ENTITY_STATE_HOLDER)
		}
	}
	...
	fun tickStates() {
		...
		setState(RcfEntityStates.MOVE_STATE, avgVelocity >= motionThreshold && limbSwingAmount != 0f)
		setState(RcfEntityStates.EATING_STATE, entity.isUsingItem && entity.useItem.get(DataComponents.FOOD) != null)
	}
