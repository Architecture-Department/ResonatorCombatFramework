...
import architecture.resonator_combat_framework.animation.data.shouldBlend
...
class AnimationController<T : Entity> @JvmOverloads constructor(
	...
	final override var state = IEntityAnimationController.State.IDLE; private set
	...
	final override var extraModel: GeometryModel? = null; private set
	...
	override val isFadingOut: Boolean get() = state == IEntityAnimationController.State.FADING_OUT
	override val isFadingIn: Boolean get() = state == IEntityAnimationController.State.CROSSFADING
	...
	override fun isActive(): Boolean = state != IEntityAnimationController.State.IDLE
	...
	override fun pause() {
		if (state == IEntityAnimationController.State.PLAYING || state == IEntityAnimationController.State.CROSSFADING) {
			state = IEntityAnimationController.State.PAUSED; transitionSource = null
		}
	}
	...
	private fun snapshotTransitionSource() {
		if (state == IEntityAnimationController.State.IDLE || poseData.bones.isEmpty()) return
...
//			fromBone.rotation.lerp(toBone.rotation, fadeProgress, toBone.rotation)
	...
	private fun checkPlaybackBounds() {
		if (state != IEntityAnimationController.State.PLAYING) return
	...
	final override fun tick() {
		...
		checkPlaybackBounds()
		tickBlend()
		if (state == IEntityAnimationController.State.PLAYING) {
	...
	private fun forceClear() {
		...
		manager.clearEmittersFor(id)
		state = IEntityAnimationController.State.IDLE
		fadeProgress = 0f
		fadeTarget = 0f
		poseData.bones.clear()
		transitionSource = null
