...
@AllOpen
class MolangDataHolder {

	/** 全局可读写变量层 */
	private val vars = HashMap<String, DoubleSupplier>()

	/** 由子类（MolangScope）提供的局部变量查找 */
	protected fun resolveLocal(name: String): Double? = null

	/** 由子类（MolangScope）处理的局部变量赋值，返回 true 表示已由子类处理 */
	protected fun assignLocal(name: String, value: Double): Boolean = false
	...
	companion object {
		...
		@JvmStatic
		fun of(holder: Any?): MolangDataHolder {
		...
		@JvmStatic
		fun MolangDataHolder.initEntityQueries(entity: Entity) {
