...
object ResourceReloadRegistry {
	...
	fun registryHighest(event: AddReloadListenerEvent) {
		RcfRegistries.getStaticAnimations(false).clear()
	}
	...
	fun registry(event: AddReloadListenerEvent) {
		...
		RcfStaticAnimations.init(false)
	}

	@SubscribeEvent(priority = EventPriority.LOWEST)
	fun registryLowest(event: AddReloadListenerEvent) {
		BedrockAnimationRegistry.getInstance(false).getAllStaticAnim().forEach { (_, animation) ->
