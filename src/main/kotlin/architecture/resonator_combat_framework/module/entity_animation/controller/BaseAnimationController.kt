...
abstract class BaseAnimationController @JvmOverloads constructor(
	...
	/** 上一游戏刻的骨骼快照，用于渲染帧 partialTick 插值 */
	val prevProxyModel = ProxyModel("prev")
...
// ---- crossfade 过渡
	 ...
	 * 不需要参数，每次调用自动推进 1 game tick。
	 */
	override fun tickAdvance(animationMapper: IAnimationMapper) {
		tickHandler(animationMapper)
		if (state == State.IDLE || state == State.PAUSED) return
		savePrevProxySnapshot()
		checkPlaybackBounds()
		advanceTickCount++
	...
	/**
	 * 渲染帧插值（每帧）：平滑混合过渡 + 跨动画骨骼插值 + 状态转换。
	 * @param partialTick 当前渲染 partialTick
	 * @param deltaSec 距上一帧的秒数，用于 smooth blend
	 */
	fun tickRender(partialTick: Float, deltaSec: Float) {
	...
	private fun savePrevProxySnapshot() {
		prevProxyModel.bones.clear()
		for ((name, bone) in proxyModel.bones) {
			val copy = ProxyBone(name)
			copy.pos.set(bone.pos)
