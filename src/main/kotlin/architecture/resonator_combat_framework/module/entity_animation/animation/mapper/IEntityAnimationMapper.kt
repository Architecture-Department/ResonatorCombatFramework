...
interface IEntityAnimationMapper<T : Entity, M : EntityModel<T>> {
	...
	fun playSoundEffect(sound: BakingBrAnimationSound, bonePos: Vector3d, molangData: MolangData?) {
		if (!isClient) return
		sound.apply(holder, bonePos, molangData)
	}

	/** 播放粒子（坐标基方法，仅客户端）。bonePos 为已从动画模型解析的骨骼坐标 */
	fun playParticleEffect(particle: BakingBrAnimationParticle, bonePos: Vector3d, molangData: MolangData?) {
		if (!isClient) return
