...
interface IEntityAnimationMapper<T : Entity, M : EntityModel<T>> {
	...
	fun playSoundEffect(sound: BrAnimationSound, bonePos: Vector3d, molangData: MolangData?) {
		if (!isClient) return
		sound.apply(holder, bonePos, molangData)
	}

	/** 播放粒子（坐标基方法，仅客户端）。bonePos 为已从动画模型解析的骨骼坐标 */
	fun playParticleEffect(particle: BrAnimationParticle, bonePos: Vector3d, molangData: MolangData?) {
		if (!isClient) return
	...
	/**
	 * 从 EntityModel 中解析骨骼的世界空间位置。
	 * 默认实现使用持有者位置，子类应覆写实现具体的 ModelPart 查找。
	 */
