package architecture.resonator_combat_framework.event.listener.client
...
object RenderEvents {
	...
	fun onRenderLivingPost(event: RenderLivingEvent.Post<*, *>) {
		...
		if (!ActionHolder.has(entity)) return
		...
		for (actionController in actionHolder.getAll().values) {
			val action = actionController.action as? AttackAnimationAction ?: continue
			val mapper = entity.getMapperProvider()
			val controller = mapper.getController(action.controllerId) ?: continue
