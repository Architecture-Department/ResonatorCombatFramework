...
/**
 * 运行时几何模型——由 [GeometryModel] 构建的可变模型实例。
 *
 * 支持骨骼和定位器的动态合并、替换和变换矩阵计算。
 */
data class DynamicGeometryModel
@JvmOverloads
constructor(
	...
	fun overwriteAdd(model: GeometryModel?) {
	...
	/**
	 * 合并 [GeometryModel] 的骨骼和定位器到当前模型。
	 * 骨骼合并不覆盖已有（仅向已有骨骼添加 cubes/locators），
	 * 定位器按 [overwriteLocators] 决定是否覆盖。
	 */
	private fun mergeBones(model: GeometryModel?, overwriteLocators: Boolean) {
		if (model == null) return
		for (bone in model.bones.values) {
	...
	companion object {
		/**
		 * 从 [GeometryModel] 构建 [DynamicGeometryModel] 实例。
		 *
		 * @param geometryModel 源几何数据
		 * @return 新的 [DynamicGeometryModel] 实例
		 */
		@JvmStatic
		fun of(geometryModel: GeometryModel): DynamicGeometryModel {
			return DynamicGeometryModel(
				geometryModel.bones.map { (k, v) -> k to BrBone.of(v) }.toMap().toMutableMap(),
				geometryModel.locators.map { (k, v) -> k to BrLocator.of(v) }.toMap().toMutableMap()
			)
		}
	}
}